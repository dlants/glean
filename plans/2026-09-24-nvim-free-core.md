...
- Status: done.
  ...
  - Review follow-up: `AppUi.cursorFile` returns `lnum: WorktreeLnum` (branded in the nvim adapter); `gotoSource`/`sourceLineRow` take that `WorktreeLnum` (nearest post-image line by number), so the `as PostLnum` cast is gone. `App.jump` picks its root from an explicit `current | open(ctx)` union instead of a `?? ""` fallback.
