...
- `skills/glean-review/skill.md` — **new**: the agent-facing skill.
...
- The skill ships in this repo (`skills/glean-review/`) so it lives with the code it documents;
  `~/src/dotfiles/nix/magenta-skills.nix` grew an `externalSkills` map that symlinks it into
  `~/.claude/skills` from `~/src/glean/skills/glean-review`.
