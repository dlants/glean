...
- Status: done. Summary section, summary-row actions and the remaining reload cases landed last (see 4d notes). `init.lua` is 5.5k lines (~190 functions) with a 4.3k-line `init_test`, too large for one pass. Split into sub-stages, each committed green:
...
- 4b progress (partial): `node/render/markers.ts` + `markers.test.ts` port `hunk_marker_runs` (`hunkMarkerRuns`, 0-based indices), `display_seen_map` (`displaySeenSet`) and `marker_key`/`cmarker_key` (one `markerKey(scope, …)` returning a branded `MarkerKey`); all `marker_test` cases pass. Deviation: only add lines contribute marker lnum bounds, because node dels carry a `newLnum` slot that Lua's test fixtures lacked.
  - `node/render/render.ts`: pure `render(RenderInput) -> Frame {lines, rows: RowTarget[], highlights, sections, intraBlocks}`. Ports `Session:build` minus comments (comment rows and the summary section wait for 4c) and minus the `pending` loading state (lineage is eager). `RowTarget` is a union (`mode-header | commit-header | dir | file-header | seen-section | divider | hunk-header | line | marker | marker-line | blank`) with a `FileRef` scope-tagged file address; `li` is 0-based. Collapse state is a `Map<CollapseKey, boolean>` of explicit overrides (branded keys built by `keys.*`, same strings as Lua); defaults are computed at render (commit/file/dir: seen; combined file: expanded; seen section and markers: collapsed), replacing Lua's mutable `collapsed` fields and `apply_collapse`. Intra-line refinement is not applied in `render`; `intraBlocks` are handed to `runRefine` by the view (4d). Tests in `render.test.ts` (3 cases). Remaining for 4b: port the render cases of `init_test`. Refine cache done: `RefineCache` in `node/git/scheduler.ts` (content-addressed by del/add text, bounded at 5000 entries with oldest-first eviction), passed as an optional arg to `runRefine`; the view (4d) owns one per session.
- 4c progress (partial): hunk-header/line rows now carry `sec: "seen" | "unseen"`. `node/render/actions.ts` holds pure planners: `targetIds` (the identities a row addresses), `planToggleSeen` (returns `SeenPlan {op, ids, sticky, clear}`; the section decides the op, headers fall back to all-seen; marker rows unmark their run's seen lines; `clear` lists the seen/dir overrides to drop so marked content re-collapses), `collapseTarget`/`toggleCollapse` (flip the effective state against the render default), and `hunkKey`/`nextUnseenHunk`/`rowOfHunk` for cursor landing. Tests in `actions.test.ts`. Undo/redo: `Session` owns `collapse` (ephemeral overrides) and an `Undoable` (`seen | collapse`) undo/redo stack; `perform`/`undo`/`redo` apply a `SeenPlan` via `applySeen(ids, op, sticky)` (sticky add/remove saved in the WORKTREE shard even when the seen write is a no-op). `plan.clear` is applied on perform only, not reversed on undo. Tested in `session.test.ts`. Scope-toggle cursor anchor: `node/render/anchor.ts` (`cursorAnchor`, `restoreAnchor` → row; exact identity, then same owner's nearest, then nearest display line, then file header), tested in `anchor.test.ts` (identity round trip across scopes, header case). The collapsed-destination expansion case of `scope_cursor_test` waits for the view (4d: expand the path, re-render, then restore). `reviveDest` (port of `revive_dest_key`) is done and tested in `actions.test.ts`. Visual range: `planVisualMark(cls, scope, rows, srow, erow)` (port of `mark_visual_range`'s id/sticky collection; cursor anchoring after the collapse is view work for 4d), tested in `actions.test.ts`. Comment placement: `node/render/comments.ts` `resolveComments(display, canonical, records)` (port of `Session:resolve_comments`; keyed by 0-based display line index, outdated records fall back to the nearest post-image line, canonical matches hidden by whitespace mode are omitted), tested in `comments.test.ts`. Comment rows: `RenderInput.comments(ref, file)` supplies placements (from `resolveComments`; `Session.commentsHook()` builds it from store records against the canonical file of the same path; tested in `session.test.ts`); `render` emits a `comment` `RowTarget` (plus a reply row) after each plain line, tested in `render.test.ts`. Comments on lines inside a seen marker run are not shown (Lua parity unverified). Comment edits: `Undoable` gained `comment {path, before, after}` (add = no before, delete = no after, edit/reply = both); `Session.swapComment` removes by id and re-adds preserving the id, saves the WORKTREE shard, reclassifies; tested in `session.test.ts`. Summary section: `collectComments(pairs, recordsFor, worktreeLines, ignoreWhitespace)` in `node/render/comments.ts` (port of `collect_comments`; `diff | file | outdated` state plus `hidden`, duplicates keep the best-ranked copy), `Session.commentSummary()` (diff paths plus store comment paths; preloads work-tree text so classification is synchronous), and `RenderInput.summary` rendered by `render` as `summary-header`/`summary-file`/`summary-comment` rows (wrapped at 100 cols); tested in `session.test.ts`. The summary-row actions (`<CR>` reveal, visual delete) go with 4d. The header-toggle case of `dirty_combined_test` is ported (render → `planToggleSeen` on the file-header row → `Session.perform`).
...
- 4d progress (partial): `node/view/view.ts` `ReviewView` renders a Session into a scratch buffer. `lineEdit` finds the shared prefix and suffix, writes the replacement in batches of at most 500 lines, then clears the namespace and re-applies the highlights in `nvim_call_atomic` batches of at most 1000. `Action` (`toggle-seen | visual-mark | toggle-fold | toggle-scope | undo | redo`, 0-based rows) is parsed at the boundary by `parseAction`. `:GleanNode open [base]` (in `glean.ts`) builds a worktree Session: repo root = nvim cwd, state dir = `vim.g.glean_state_dir` or `stdpath("data")/glean-node/<sha256(root)[:16]>`. This is a deviation, since it does not use Lua's `repo_state_dir`; reconcile at cutover. It then starts live polling. Lua `glean.node.open_review_buffer` creates the buffer, and its keymaps (`m`, visual `m`, `=`, `S`, `u`, `<C-r>`) are each one `gleanAction` rpcnotify. Driver tests are in `view.driver.test.ts` (the `m` round trip and the `S` scope toggle).
  ...
  - Still to do for 4d:
    - Highlight diffing: done, per row rather than per section. `frameRowKeys` keys each row by text plus its highlights; `lineEdit` runs over those keys, so only the changed span is rewritten, its namespace range cleared and its highlights re-applied (extmarks outside the span move with their lines).
    - First-poll baseline gap: fixed. `Session.refresh` seeds the poll/untracked signatures concurrently with `buildModel`, so an edit after the refresh starts is caught on the next tick.
    ...
    - Summary section and summary-row actions: done. `ReviewView.draw` awaits `Session.commentSummary()` before each render and passes it as `summary`. New `Action` variants: `reveal-comment {row}` (`<CR>` on a `summary-comment` row lands on the inline comment row, first expanding `revealKeys` for the path in every owning commit if it's hidden) and `delete-comments {srow, erow}` (visual `d`: one undoable `comment` delete per summary-comment row). Driver test "summary <CR> reveals the comment and visual d deletes it".
    ...
    - `reload_test`: done. `reload.test.ts` also covers idle poll (no refresh, no onChange, no log), untracked files (only the untracked tick picks them up) and ignore-whitespace live refreshes (a whitespace-only save stays hidden from display but kept in `canonicalFiles`; a later semantic edit shows with real coordinates). Not ported: the history-cache-per-whitespace-mode counts (covered by the `CommitPatchCache` design), diffsplit/jump (deferred UI), and the `:e` hard reset (cutover).
    - `init_test`: decision to stop here. Its model, render, action, marker, scope, comment and summary behaviour is covered by the node suites above. The cases left are UI that was deferred (LogView/PrView/jump/diffsplit/sticky float, the comment editor) or Lua internals with no counterpart in node. Port them with those features at cutover.
    - Review follow-up: `LineOwner` is split by side (`commit-add` PostLnum, `commit-del` PreLnum, `worktree-add` WorktreeLnum, `worktree-del` HeadLnum), branded once in `layerOwner`, so `lineIdentity` has no casts. The `comment` Undoable carries `change: add | delete | edit` rather than two optional records. `parseOpenConfig` validates the `nvim_exec_lua` config tuple and maps vim.NIL to undefined. Row/anchor owner shas are typed `Layer | undefined`. Added tests: `collectComments` dedup and whitespace-`hidden`, `resolveComments` with a distinct canonical, and `parseCommand` `open`.
    - `vitest.config.ts` now sets `testTimeout: 30000`: under parallel load, the git-spawning suites went past the 5 s default.
...
- Status: done. Code in `node/gutter/` and `lua/glean/node_gutter.lua`.
  - `project.ts` (pure): `project` → `GutterMarks` (`Map<lnum, {kind: add|change|del|context, delBelow, delAbove, seen, sources}>`, 0-based `{hunk, li}` sources; a del-only row is `kind: "del"` instead of Lua's nil kind), `hunkStarts`, `hunkRange`, `nextHunkRow`. `project.test.ts` ports every pure `gutter_test` case.
  - `marking.ts` (pure over a `Classifier`): `fileStatus(cls, path)` (combined scope, work-tree reviews only) and `planFileMarks(cls, path, srow, erow, expandHunk)` → `ok {plan: SeenPlan} | inert {reason}` (port of `Session:toggle_marks`). `marking.test.ts` ports `toggle_mark_test` (range/polarity, partial completes, hunk expansion, inert rows/paths, uncommitted dels recorded as explicit head ranges). The "ownership still loading" case doesn't exist (lineage is eager); the foreign-window cursor case has no counterpart.
  - `bufUndo.ts`: `BufUndo<A>` stacks per buffer keyed by `seq_last`; a moved seq (novel edit) wipes them. Node owns the stacks and mirrors `{undo, redo, seq}` into `b:glean_undo`; the Lua `u`/`<C-r>` maps read that var (O(1)) to decide between a `gleanGutter` undo/redo notification and plain text undo/redo, so the fallthrough stays synchronous in nvim.
  - `fileGutter.ts` `FileGutter`: handles `gleanGutter` events (`refresh | focus | toggle-mark | goto-hunk | undo | redo | toggle | wipe`, parsed by `parseGutterEvent`) serialized on one promise chain. Each event does one `node_gutter.info(bufs)` query (name, modified, line count, cursor, `seq_last`, focus flag), then paints signs/focus/stale placeholders plus the Lua `attach`/`detach` calls in bounded `nvim_call_atomic` batches. `Session.subscribe` (new, alongside `onChange`) repaints every buffer when the model changes. `toggle-mark` refuses modified buffers, pokes one poll, then compares buffer text to disk (replacing `wt_matches`, since the node model doesn't keep W).
  - Lua `node_gutter.lua`: autocmds (BufReadPost/BufWinEnter/BufWritePost/FileChangedShellPost, TextChanged only when modified, CursorMoved/WinEnter only on member buffers, BufWipeout), the per-buffer maps (`]c`, `[c`, `gm`, `gmm`, `gmc`, visual `gm`, `u`, `<C-r>`), the global `gt` toggle, highlight links, `setup(cfg)` (`suppress` may be a `{detach, attach}` table, so it stays in Lua), gitsigns detach/attach, and a `teardown` that the bridge teardown calls (reattaches providers, clears both namespaces). `:GleanNode toggle-mark` takes a range; `:GleanNode toggle-gutter` flips the global switch.
  - Deviations: `gt` with no live review does not open one in the background (`open_dirty` belongs to cutover). The gutter follows the most recently opened review (`liveSession` in `glean.ts`). If a poll tick is already in flight, `pokePoll` doesn't wait for it, so a mark right after a write can still be refused as out of date; the reviewer just tries again.
  - Driver tests in `gutter.driver.test.ts`: paint/stale/revert, `gmm`/`gm2j` plus `u`/`<C-r>` on the mark stack with cursor parking, write-then-mark and a novel edit wiping the stack, `gmc` plus the focus overlay, `]c`, per-buffer and global toggles, the uncommitted-deletion glyph through `:GleanNode toggle-mark`, and fake-provider suppression with reattach and sign clearing when the backend exits.
  - Review follow-up: `node_gutter.info(bufs, with_seq)` only runs `undotree()` when node asks (toggle-mark), so the all-buffer scan stays cheap. `FileGutter` coalesces `refresh` events (one queued repaint per buffer), and `refreshAll` carries a generation: a superseded full repaint is skipped, and a running one yields (`setImmediate`) between buffers and abandons once stale. `pokePoll` inside the chain is bounded because every git call has a timeout. Gutter line numbers are `WorktreeLnum` (`GutterMarks`, `hunkStarts`/`hunkRange`/`nextHunkRow`, `planFileMarks` rows, event rows, cursor; branded at `parseGutterEvent`/`parseInfos` and once in `project`). `fold` takes a `FoldUpdate` union. `nvim_buf_get_lines` output is checked, the atomic-batch `as never` is gone, and the nil for "every buffer" is produced only at the RPC call. New tests: reversed range in `marking.test.ts`, `subscribe` unsubscribe in `session.test.ts`.
...
- Status: done.
  ...
  - Review follow-up: repo mode caches the `rev-parse --show-toplevel` root per probe path; the store is still re-read per call (bounded file IO) because live sessions write the same shards. The post-write session refresh is fire-and-forget. Mutating calls (`mark`, `add_comment`, `reply`, `unreply`) are deduped by name+args: a retry joins an in-flight call, and a call that answered PENDING stays joinable for 60 s after it lands, so a retry doesn't apply twice. `add_comment` validates the path (`parseRepoPath`: no absolute paths or `..`) and omits `origin` when HEAD can't be resolved (instead of casting WORKTREE to `Sha`). `HunkEntry.kind` is `FileKind`, the hunk-mode cast is gone, `Api.call` returns undefined and the RPC handler in `glean.ts` maps it to nil. The driver test covers the non-repo error through the Lua shim. Deferred nit: `ApiComment` keeps `lnum: number | undefined` plus `outdated` (the JSON shape agents read, per the skill doc).
...
- Status: done.
  - Deleted every Lua implementation module and test, `run_tests.lua`, `testutil.lua`, and `node/core/fixtures/generate-lua-store.lua` (it required the deleted `glean.state`; the Lua-written fixture shards stay as frozen fixtures). `node_api.lua` became `lua/glean/api.lua`, so agents keep calling `require("glean.api")`.
  - `lua/glean/init.lua` is now only `setup`: config (`default_base` = `HEAD`, `ignore_whitespace`, `min_seen_run`, `gutter`) copied into `vim.g.glean_*`/`node_gutter.setup`, highlight groups (re-applied on ColorScheme), and `require("glean.node").start()`. The bridge registers `:Glean` (`nargs = "*"`; a bare `:Glean` sends `open <default_base>`). Augroups were renamed `GleanBridge`/`GleanReview<buf>`.
  - Store location reconciled with Lua: `<stdpath data>/glean/<sha256(git common dir)[:16]>` with worktree shard `WORKTREE/<branch or HEAD>` (`storeLocation` in `glean.ts`), so stores written by the Lua version load. `vim.g.glean_state_dir` still overrides the dir. The view driver test that seeds a store now writes the branch shard.
  - dist: gitignored and built by `npm run build`, as magenta does. Without a bundle (or with `GLEAN_DEV=1`) `start` runs the TS source.
  - README, `doc/glean.txt` and `context.md` were rewritten for the node architecture and the features that exist now. Features the Lua version had that were not ported (see the stage 4 deferrals) are dropped from the docs: `:Glean log`/`prs`/PR reviews/`<base> <target>`, `jump`, diffsplit `D`, the sticky float, `W` in-place whitespace toggle, the comment editor (`c`/`i`/`dd`), `M`/`U`/`ac`, `]c`/`]f` in the review buffer, and the file-buffer comment overlay/`:Glean comments`.
  - Manual review: `dlants-monolith-temporal-eks` and `dlants-ddt-simplification` are not checked out on this machine, so they could not be reviewed here. Instead a headless smoke test using the built bundle ran `setup` → `:Glean open HEAD~3` on this repo: a 501-line review painted in about 220 ms, and `require("glean.api").sessions()` answered. The two hang repos still need a manual check by the user.
  - Review follow-up: the path computation is split out as pure `storePaths` in `glean.ts`, unit-tested in `glean.test.ts` against a known commonDir hash (matches `vim.fn.sha256`) plus the `<data>/glean` fallback, `WORKTREE/HEAD` for failed/empty/detached branch lookups, and the override. A new view driver test writes a comment through `glean.api.add_comment` (`repoContext`) and then opens a review with a bare `:Glean` (`default_base`), which checks that both pick the same shard and covers the no-argument default.
