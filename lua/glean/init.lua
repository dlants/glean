...
-- Is this path declared generated by the repo's `.gleanignore`? Generated files
-- are derived-seen rather than marked: the store never learns about them, so
-- dropping a pattern makes the file unseen again with no state to unwind.
function Session:is_generated(path)
  return path ~= nil and self._ignore ~= nil and self._ignore:match(path)
end
...
function Session:id_seen(id)
  if self:is_generated(id.path) then return true end
  if id.kind == "wt" then
...
function Session:refresh_model(hints)
  ...
  build_model_async(self.git, self.base, self.target, {
    ...
    self._ignore = ignore_mod.load(self.git.repo_root)
    self._wt_lines = nil
