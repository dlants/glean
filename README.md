...
Requires Neovim 0.12+, node 22+ and git. Install with Neovim's native plugin manager (`vim.pack`):
```lua
...
```
glean has no external plugin dependencies. Without a built `dist/glean.mjs` (or with `GLEAN_DEV=1`) it runs the TypeScript source directly with `node --experimental-transform-types`.
